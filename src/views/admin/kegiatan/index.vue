...
const filtered = computed(() =>
  store.list.filter((k) => {
    const matchSearch =
      ...
      k.coordinator.toLowerCase().includes(searchQuery.value.toLowerCase())
...
<template>
  <div class="crud-page">
    ...
    <div class="mini-stats">
      <div class="mini-stat">
        <span class="ms-val">{{ store.list.length }}</span
        ><span class="ms-label">Total</span>
      </div>
      <div class="mini-stat">
        <span class="ms-val green">{{
          store.list.filter((k) => k.status === 'active').length
        }}</span
        ><span class="ms-label">Aktif</span>
      </div>
      <div class="mini-stat">
        <span class="ms-val amber">{{
          store.list.filter((k) => k.status === 'inactive').length
        }}</span
        ><span class="ms-label">Nonaktif</span>
      </div>
    </div>
    ...
    <div class="kegiatan-grid">
      <div
        ...
        class="empty-state"
        style="background: white; border-radius: 16px; grid-column: 1/-1"
      >
        <span>📅</span>
      ...
      >
        <div class="kegiatan-img" :style="{ backgroundImage: `url(${item.image})` }"></div>
        <div class="kegiatan-top">
          <button
            class="status-toggle-btn"
            :class="item.status"
            @click="store.toggleStatus(item.id)"
